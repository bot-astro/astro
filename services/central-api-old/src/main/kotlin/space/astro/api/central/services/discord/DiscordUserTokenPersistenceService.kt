...
class DiscordUserTokenPersistenceService(
    ...
    fun upsert(userId: String, tokenPayloadDto: TokenPayloadDto) {
        val tokenEntity = DiscordUserTokenEntity.fromTokenPayload(userId, tokenPayloadDto)
        discordUserTokenRepository.save(tokenEntity)

        redisTemplate.opsForValue().set(
            RedisKey.DISCORD_USER_TOKEN(userId),
            jsonMapper.writeValueAsString(tokenPayloadDto),
            (tokenPayloadDto.expiresIn - 300).seconds.toJavaDuration(),
        )
    }
