package space.astro.bot.interactions.button.impl.vc.ownership
...
class TransferButton(
    ...
    suspend fun run(
        ...
        if (members.isEmpty()) {
            event.replyEmbeds(Embeds.error("Cannot find a user in your voice channel that is not a bot.\nRun this command again when someone joins your voice channel"))
                .setEphemeral(true)
                .queue()
        ...
        event.replyEmbeds(Embeds.default("Choose the new owner of your voice channel with the menu below"))
